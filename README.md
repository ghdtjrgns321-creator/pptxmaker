# 에이전트 기반 PPT 서식 통제·상속 시스템⚠️readme 수정 진행 중

---

## 개요

**골든 덱**을 기준으로 서식과 밀도를 기계 게이트로 강제하는 결정론적 PPT 빌드 시스템<br>
<br>
덱을 생성하거나 수정할 때마다 드러난 미비점(버그, 배선 공백, 부품 부재)을 deck-smith 에이전트가 즉각 반영 (부품>게이트>배선>훅)<br>
수정된 규칙이 다음 생성물에 영구적으로 상속되는 지속적 개선 루프

1. Grill    : 사용자와 목차·콘텐츠 등 내용 인터뷰
...
LG생활건강에서 보고서와 발표자료를 만들며 겪은 어려움에서 출발<br>
...
> 아래는 **골든 덱 생성 공장 루프 과정 예시**이다(정체 §0의 루프 실증).
...
━━━ S16 생성  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  ◆ 시안       하단 한계 3장을 손으로 그림 — 번호 + 제목 + 회색 텍스트
                     │
                     ▼
  ◆ 반려       글자수·채움률·넘침 게이트는 전부 green인데 눈으로는 밋밋하고 허전한 공간이 많음
                     │
                     ▼
  ◆ 원인       "카드 = hero_card 단일"(리치 컴포넌트)이 규칙·부품으로 있었으나
                     │            **기계 강제가 없어** 불완전한 카드가 통과됐다
                     ▼
  ◆ 배치       check_adhoc_card 신설(카드 크기 밝은 박스에 구조 자식 0 = FAIL)
                     │            통과작 8장 오탐 0 검증 → 정본 게이트에 배선(전 경로 강제)
                     ▼
  ◆ 재건       hero_card로 다시 조립 → 게이트 통과. 통과되지 않은 카드는 preflight에서 막힌다
                     └─ 발견이 코드로 내려가 다음 덱이 상속한다
...
생성 전 인터뷰를 통해 사용자와 목차·내용을 확정하는 단계

```
━━━ ① 인터뷰  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  ◆ 베이직라인 생성    목차 1안을 만들어 놓고 사용자 질문 시작
                       │
                       ▼
  ◆ 부·장 단위 채록    목차 구성이 사용자의 생각과 맞는지
                       각 장에서 가장 전하고 싶은 메시지가 무엇인지 
                       │             
                       ▼
  ◆ 재료 확정         확정 이미지 · 앵커 수치 · 강조점을 계약에 박음
                       │
                       ▼
      계약 파일 생성 — 이후 모든 단계에서 기준점으로 참조

```

이 계약파일은 ④ 검증에서 **정답지 역할이 됨**
...
각 장의 물성(비교인가 · 흐름인가 · 분해인가 · 실물 화면인가)을 골든 덱과 대조해 출발점을 정한다. **내용이 형태를 정하지, 형태에 내용을 맞추지 않는다.**
