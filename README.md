...
#### 3-3. 검증 — 상세 [3-3. 기술 설명 - 기계 검사](#3-3-기술-설명---기계-검사)
...
동일 계약 및 원천 자료를 기반으로 1장의 렌더링을 3회 반복 수행<br>
...
| 통제 계층     | 생성 대상                                                                       |
| ------------- | ------------------------------------------------------------------------------- |
| 공통 하네스   | 슬라이드 전체 크기, 색, 글꼴, 헤더 밑줄 위치, 목차 네비게이션, 한글 줄바꿈 규칙 |
...
- 레이아웃 다형성 강제: 각 장에 독립된 시각화 조합 적용 (동일 조합이 본문 3장에 오르거나, 한 부품이 본문의 60%를 넘으면 자동 차단)
...
> - 정형 페이지와 기본 도구 : [`kit.py`](.claude/skills/pptx-build/scripts/deckkit/kit.py)
...
### `kit.py` — 정형 페이지와 기본 도구

| 분류              | 제공 기능 상세                                                             |
| ----------------- | -------------------------------------------------------------------------- |
| 정형 페이지 골격  | 표지, 목차, 간지, 목차 네비게이션 등                                       |
...
> - 검사 호출 : [`deck_state.py`](scripts/deck_state.py)
...
| 덱 전체 어휘    | 장별로는 통과해도 덱 전체에서 아이콘이 한 번도 안 쓰인 경우                  |
...
- 변동성 발생 원인 :데이터의 성격에 기인
  - 원천 자료에 순서와 개수가 정해진 07장은 배치가 일정하게 수렴
...
| 2   | **원천 자료 의존성** | 원천 자료에 없는 정보는 생성 불가, 원천 자료의 뎁스나 내용 구성에 따라 생성 품질이 달라짐                            |
