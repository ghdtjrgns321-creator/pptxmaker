...
# §5 나열형 2레인 금지 → 분기는 공통 뿌리 + 엘보 커넥터 + 밴드. §6 2×2 매트릭스보다 스토리 구도 우선.
...
LANE_LABEL_H = 0.22
ROW1_Y = 2.60  # 라벨1 2.25~2.47 (룰 2.11에서 공기 0.14)
ROW2_Y = 3.72  # 라벨2 3.37~3.59 (ROW1 bottom 3.22에서 공기 0.15)
FLOW_H = 0.62
QW = 1.45
CONN_W = 0.4
FX = G.MARGIN_L + QW + CONN_W  # 셰브런 시작 2.45
OVERLAP = FLOW_H * 0.55 / 2
STEP_W = (G.RIGHT_EDGE - FX + 3 * OVERLAP) / 4
Q_CY = (ROW1_Y + ROW2_Y + FLOW_H) / 2  # 3.47 — 뿌리 세로중심 = 두 레인 전체 중심(§5)
QH = 1.3
B2_HEAD_Y = 4.52  # 하단 밴드 소제목 (ROW2 bottom 4.34에서 공기 0.18), 룰 4.88
PANEL_Y = 5.02
PANEL_H = G.CONTENT_BOTTOM - PANEL_Y  # 1.33 (bottom 6.35)
PANEL_GAP = 0.3
...
def variant_k(prs, c=None):
    """2밴드 — 상단: 분기(같은 질문 두 결말) · 하단: 대비(1종/2종). c: 텍스트 override."""
    c = {**DEFAULT, **(c or {})}
    ...
    # ── 상단 밴드: 분기 (공통 뿌리 → 엘보 → 두 경로 셰브런 밴드) §5 ──
    ...
    # ── 하단 밴드: 대비 쌍 (동일 지오메트리, 채움 명도만 차등) §5 ──
