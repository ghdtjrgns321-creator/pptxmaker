...
def build_full():
    ...
    BG.build_s01_cover(g); BG.build_s02_toc(g); BG.build_part(g, 1)
    g_s04(g); BG.build_part(g, 2); g_s06(g); BG.build_part(g, 3)
    g_s08(g); g_s09(g); g_s10(g); g_s11(g); g_s12(g)
    BG.build_part(g, 4); g_s14(g); g_s15(g); BG.build_part(g, 5)
    g_s17(g); g_s18(g); g_s21(g)

    f = Presentation()
    f.slide_width, f.slide_height = Inches(13.333), Inches(7.5)
    cover_c = {"kicker": BG.KICKER, "title": BG.TITLE, "value_prop": BG.VALUE_PROP,
               "year": BG.YEAR, "brand_name": FL.K["brand"]["name"]}
    toc_c = {"title": "목차", "items": BG.TOC, "deck_title": BG.TITLE, "deck_sub": "기술 제안서 · 2026"}
    def part_c(no):
        return {"no": no, "title": BG.TOC[no - 1][0], "lead": BG.PART_LEADS[no - 1]}
    FAB["cover"](f, cover_c); FAB["toc"](f, toc_c); FAB["part"](f, part_c(1))
    FAB["problem_grid"](f); FAB["part"](f, part_c(2)); FAB["exec_graph"](f); FAB["part"](f, part_c(3))
    FAB["tech_evidence"](f); FAB["tech_tree"](f); FAB["screenshot"](f); FAB["tech_mechanism"](f)
    FAB["tech_capture"](f); FAB["part"](f, part_c(4)); FAB["ab_simulation"](f); FAB["validation"](f)
    FAB["part"](f, part_c(5)); FAB["mirror_matrix"](f); FAB["boundary"](f); FAB["closing"](f)

    names = ["S1 표지", "S2 목차", "S3 간지1", "S4 문제정의", "S5 간지2", "S6 실행그래프",
             "S7 간지3", "S8 용어사전", "S9 지식그래프", "S10 스크린샷", "S11 판단트리",
             "S12 구조화출력", "S13 간지4", "S14 트러블슈팅", "S15 골든테스트", "S16 간지5",
             "S17 미러매트릭스", "S18 경계", "S19 클로징"]
    return g, f, names
...
def param_efficacy():
    ...
    f = Presentation()
    f.slide_width, f.slide_height = Inches(13.333), Inches(7.5)
    FAB["closing"](f, {"value_prop": "테스트 가치제안 문장 A" + chr(10) + "둘째 줄 B", "kicker": "CLOSING — 테스트덱"})
    texts = " ".join(sh.text_frame.text for sh in f.slides[0].shapes if sh.has_text_frame)
    results.append(("closing 커스텀", "테스트 가치제안 문장 A" in texts and "CLOSING — 테스트덱" in texts))
    ...
    results.append(("screenshot 커스텀", "커스텀 헤드라인 검사용" in texts2 and "커스텀 캡션 — 검사" in texts2))
    return results
...
def main():
    ...
    for gs, fs2, name in zip(g.slides, f.slides, names):
        ...
        lines.extend(rows)
        total_mism += mism; total_ok += ok; total_n += n
    lines.append("")
    lines.append(f"총계: 슬라이드 {len(names)}/19, 도형 일치 {total_ok}/{total_n}, 불일치 {total_mism}건")
    lines.append("")
