#!/usr/bin/env python
"""Lucide(MIT) SVG 아이콘을 브랜드 색 PNG로 변환해 assets/icons/에 박제한다.

색은 `goldenfab.kit.load_kit()`에서 온다 — 값의 단일 출처는 `deckkit/brand.yaml`이다.
...
def main():
    argparse.ArgumentParser().parse_args()  # 인자 없음 — 브랜드는 단일 출처에서 온다
    build = Path(__file__).parents[2] / "pptx-build"
