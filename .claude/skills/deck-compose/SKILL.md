...
## 부품·틀 선택 (v7 — 2026-07-27 · 구조·정성 물성은 **여기가 1순위**)

산문 결정표보다 **먼저** 돈다. 골든 도해가 부품으로 나와 있으므로 장 통째 배정 대신 부품을
꺼내 틀에 앉힌다 — 장 통째 배정이 "실전 덱 타입 분포 = 골든 SLIDE_ORDER" 복제의 기계적
원인이었다(`docs/PIPELINE.md`).

```
uv run python .claude/skills/pptx-build/scripts/pick_parts.py <재료.json>
```

재료 JSON은 장마다 **셀 수 있는 것만** 적는다(그림 이름을 적지 않는다 — 적으면 판정을 앞으로
숨긴 것이지 기계화가 아니다):

```json
{"slides": [{"no": 4, "title": "...", "items": [...], "sets": 1,
             "links": [["a","b"]], "ordered": false, "extra": true,
             "head": null, "tail": null,
             "figures": 2, "cards": 4, "columns": 0, "band_heads": 2}]}
```

러너가 축 5개(`묶음수·방향·순서·부가열·양끝`)를 세어 부품 후보를 좁히고 **2단 규칙**
(① 덱 내 미사용 → ② 개수 적합도)으로 하나를 고른다. 자리는 `frames.py`의 틀 5종에서
재료가 성립시키는 것만 후보로 나온다. 규칙 본체는 `goldenfab/select.py`·`frames.py`.

**후보가 0이면 멈춘다.** 가장 가까운 부품으로 때우지 않는다 — 그렇게 만든 도해는 다음 덱이
상속하지 못하고 한 번 쓰고 버려진다. 러너가 `exit 1`과 함께 막힌 장 목록을 내고, 그 목록이
곧 `part-design`으로 만들 부품 목록이다(**지어내지 않고 수요로 만든다**).

정량 물성(시계열·구성비·분포 등)은 부품에 어휘가 없다 — 아래 산문 가이드와 L-ID가 정본이다.

> **회귀 검증:** `uv run python .claude/skills/pptx-build/scripts/verify_selection.py`
> — 골든이 손으로 고른 도해 8장을 규칙이 재현하는지 대조한다(2026-07-27 실측 8/8 일치).
> 부품이 늘어 축이 창고를 못 가르게 되면 이 검사가 알려준다.
