# 시안 J — 색 규율·그리드 정합·타이포 점프·강조 통일 (I 계승)
from pptx.enum.shapes import MSO_CONNECTOR
from pptx.enum.text import MSO_ANCHOR, PP_ALIGN
from pptx.util import Inches, Pt

from _variant_h import BAND_H, _shape_step
...
def _zone_title(slide, x, y, w, text):
    ...
    add_box(slide, x, y + 0.38, w, 0.012, fill=C["muted"])


def variant_j(prs):
    """J안(v11) — 흑/백/회 + accent 3곳, 베이스라인 공유 그리드, 타이포 점프 확대."""
    ...
    nums = [
        ...
        (
            "02",
            "판단 창작",
            "기준서에 실재하지 않는 기준을 확신에 찬 어조로 창작한다.",
        ),
    ]
    for i, (no, head, desc) in enumerate(nums):
        ny = Y["zone2_title"] + i * 0.88
        add_text(slide, AX, ny, 0.5, 0.3, no, S["head"], F["head"], C["muted"], bold=True)
        add_text(
    ...
    for lbl_y, ly, name, ink, steps, verdict in lanes:
        add_text(slide, FX, lbl_y, 3.0, 0.24, name, S["caption"], F["head"], C["primary"], bold=True)
