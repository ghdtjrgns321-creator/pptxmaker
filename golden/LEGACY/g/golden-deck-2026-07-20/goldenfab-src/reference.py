"""goldenfab.reference — 골든 레퍼런스 덱(17장·5부) 정의. 회귀 게이트의 기준 산출물.
...
PART_LEADS = [  # 간지 리드 — 00_factsheet.md(실데이터 v2) 부별 요약
    ...
    "실측된 네 균열마다 구조적 대체물을 배치했습니다 — 확률 신호의 전량 폐기, 그리고 정직한 한계",
    "정답 문서를 차단한 채 사람 전문가의 결론을 78/92 재현했습니다 — 수치가 실제 능력을 반영합니다",
]

# 2026-07-16 5부 개편(사용자 확정): 차별점 부 삭제 — S17(7축 비교)은 셀 전부가 다른 장에
# 실물로 있는 요약 재탕이라 삭제하고 주장만 S14 결론 바에 흡수, S18(정직한 한계)은 자기가
# 인용하는 수치(92건·59.1%·홀드아웃)가 먼저 나오는 검증 장 뒤(Part Ⅳ)로 이동.
TOC = [  # (제목, 서브카피 ' · ' 구분, 시작 페이지) — 00_factsheet.md §A~§H
    ...
    ("문제와 해결", "확률 신호의 실패 실측(탈락 103/105) · 재현 72→78 · 정직한 한계", "13"),
    ("성과·증거", "홀드아웃 78/92 · 에러 0건 · 미재현 원인 분해", "15"),
]

# 장 순서 — (레이아웃 키, 리포트용 이름). content가 None이면 그 레이아웃의 골든 기본값 사용.
# mirror_matrix는 골든 덱에서 제외됐지만 레지스트리 타입으로는 유지된다(실전 덱 창고).
SLIDE_ORDER = [
    ("cover", "S1 표지"),
    ("toc", "S2 목차"),
    ("part", "S3 간지1"),
...
def part_content(no):
    # total을 명시하지 않으면 layouts.part가 기본 6으로 점을 찍는다 — 5부 덱에 6점은 조용한 오류
    return {"no": no, "title": TOC[no - 1][0], "lead": PART_LEADS[no - 1], "total": len(TOC)}
