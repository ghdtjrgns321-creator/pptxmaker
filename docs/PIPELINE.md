...
| 부품으로 꺼낸 도해       | **3종** — `bipartite_map` · `hub_spoke` · `layer_stack`                |
| 눈검증 통과              | `bipartite_map` · `hub_spoke`(line 모드). 나머지 모드·부품은 판정 대기 |
| 골든 장에 부품이 물린 수 | **0** — 장 함수(`sNN_dense.py`)가 아직 정본. 교체는 부품 승인 후       |

즉 도구함은 **만드는 중**이고 생산 축은 아직 장 함수로 돈다. 부품이 승인되면 장 함수를
부품 호출로 바꾸고 `compare_golden` 픽셀 동일로 무손실을 증명한다.
